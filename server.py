...
from eshop.view.product_schemas import (
...
def product_create_endpoint():
    try:
        product_create_dto = ProductCreatedDtoSchema().load(request.json)
    except ValidationError as err:
        return err.messages, 400

    try:
        product = product_create(product=product_create_dto["product"])
    except Exception as e:
        return {"error": str(e)}
